...
sce <- slingshot(sce, clusterLabels = 'seurat_clusters', reducedDim = toupper(red), start.clus = 2, end.clus = c(0,5))
...
TULIP_total <- length(data.QC@meta.data$orig.ident[data.QC@meta.data$orig.ident %in% 'TULIP'])
TULIP_ILC   <- nrow(compare[compare$Sample %in% 'TULIP',])
fraction    <- TULIP_ILC/TULIP_total*100 
# 61/320 -> 19.0625%

overLIP_total <- length(data.QC@meta.data$orig.ident[data.QC@meta.data$orig.ident %in% 'overLIP'])
overLIP_ILC   <- nrow(compare[compare$Sample %in% 'overLIP',])
fraction    <- overLIP_ILC/overLIP_total*100 
#918/1073 -> 85.55452%

ggplot(compare, aes(x=Slingshot, y=..scaled.., fill=factor(Sample, levels = c("ALP", "overLIP", "TULIP", "ILCpro")))) +
  geom_density(alpha=0.5, linewidth = 0.6) +
  theme_classic() +
  scale_fill_manual(values = ColorBlind12[c(6, 5, 1, 4)], name = "")

ggplot(data=compare, aes(x=Slingshot, fill=factor(Sample, levels = c("ALP", "overLIP", "TULIP", "ILCpro")))) +
...
ggplot(compare, aes(x=Slingshot, y = 1, colour=factor(Sample, levels = c("ALP", "overLIP", "TULIP", "ILCpro")))) +
