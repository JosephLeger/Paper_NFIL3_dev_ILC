...
NF.KO     <- Read10X(data.dir = paste(dir.data, "/2019_scRNAseq_data/NFIL3_KO", sep = '')) %>%
  CreateSeuratObject(project = "NF-KO", min.cells = 1, min.features = 0)
TOX.KO    <- Read10X(data.dir = paste(dir.data, "/2019_scRNAseq_data/TOX_KO", sep = '')) %>%
...
data  <- merge(TULIP, c(ILCpro, ALP, overLIP, NF.KO, TOX.KO), add.cell.ids = c("TULIP", "ILCpro", "ALP", "overLIP", "NF-KO", "TOX-KO"), project = "scRNA_Integration")
...
data[["orig.ident"]] <- factor(data@meta.data$orig.ident, levels = CellTypeOrder)
...
plot0   <- VlnPlot(data, features = c("nFeature_RNA","Percent.mt"), group.by = "orig.ident", cols = ColorBlind[c(10,1,9,3,6,12)]) & 
...
plot1   <- VlnPlot(data.QC, features = c("nFeature_RNA", "Percent.mt"), group.by = "orig.ident", cols = ColorBlind[c(10,1,9,3,6,12)]) & 
  theme(axis.title.x = element_blank())
plot1
...
data.combined <- RunUMAP(data.combined, dims = 1:17, metric = "euclidean", n.neighbors = 50)
...
plot4         <- DimPlot(data.combined, group.by = "orig.ident", reduction = "umap", cols = ColorBlind[c(10,1,9,3,8,6)], pt.size = 0.9, order = rev(CellTypeOrder)) +
  labs(title = "", x = "UMAP1", y = "UMAP2")
plot4
...
plot5 <- DimPlot(data, group.by = "Experiment", dim =c(2,1), reduction = "umap_cssz", cols = ColorBlind[c(1,10)], pt.size = 0.5, order = rev(CellTypeOrder)) +
  labs(title = "", x = "UMAP2", y = "UMAP1")
plot5

plot6 <- DimPlot(data, group.by = "orig.ident", dim =c(2,1), reduction = "umap_cssz", cols = ColorBlind[c(10,1,9,3,8,6)], pt.size = 0.9, order = rev(CellTypeOrder)) +
  labs(title = "", x = "UMAP2", y = "UMAP1")
plot6
...
plot7         <- DimPlot(data, dim =c(2,1), reduction = "umap_cssz", group.by = "seurat_clusters", label = TRUE, repel = TRUE, cols = c(rep(c(ColorBlind, 'purple'),5)), pt.size = 0.5) +
  labs(title = "", x = "UMAP2", y = "UMAP1")
plot7
...
plot_density(data, dims = c(2,1), 'Nfil3', pal = 'magma') + labs(x = 'UMAPCSSZ2', y = 'UMAPCSSZ1')

# B cells
plot_density(data, dims = c(2,1), 'Pax5', pal = 'magma') + labs(x = 'UMAPCSSZ2', y = 'UMAPCSSZ1')
plot_density(data, dims = c(2,1), 'Ebf1', pal = 'magma') + labs(x = 'UMAPCSSZ2', y = 'UMAPCSSZ1')
# Mast cells
plot_density(data, dims = c(2,1), 'Cpa3', pal = 'magma') + labs(x = 'UMAPCSSZ2', y = 'UMAPCSSZ1')
plot_density(data, dims = c(2,1), 'Gzmb', pal = 'magma') + labs(x = 'UMAPCSSZ2', y = 'UMAPCSSZ1')
plot_density(data, dims = c(2,1), 'Kit', pal = 'magma') + labs(x = 'UMAPCSSZ2', y = 'UMAPCSSZ1')
# Mono/Macro
plot_density(data, dims = c(2,1), 'Lgals3', pal = 'magma') + labs(x = 'UMAPCSSZ2', y = 'UMAPCSSZ1')
plot_density(data, dims = c(2,1), 'Lyz2', pal = 'magma') + labs(x = 'UMAPCSSZ2', y = 'UMAPCSSZ1')
# ILC2
plot_density(data, dims = c(2,1), 'Il2ra', pal = 'magma') + labs(x = 'UMAPCSSZ2', y = 'UMAPCSSZ1')
plot_density(data, dims = c(2,1), 'Icos', pal = 'magma') + labs(x = 'UMAPCSSZ2', y = 'UMAPCSSZ1')
plot_density(data, dims = c(2,1), 'Stab2', pal = 'magma') + labs(x = 'UMAPCSSZ2', y = 'UMAPCSSZ1')
...
NF <- RunUMAP(NF, reduction = "cssz", dims = 1:ncol(Embeddings(NF, "cssz")), n.neighbors = 50, reduction.name="umap_cssz2", reduction.key="UMAPCSSZ2_")
NF <- FindNeighbors(NF, reduction = "umap_cssz2", dims = 1:ncol(Embeddings(NF, "umap_cssz2")))
NF <- FindClusters(NF, resolution = 0.06)

# Create inverted embedding to simplify representation
embed           <- as.matrix(data.frame(NF@reductions[["umap_cssz2"]]@cell.embeddings[,1],
                                        -NF@reductions[["umap_cssz2"]]@cell.embeddings[,2]))
colnames(embed) <- c('REVUMAPCSSZ2_1', 'REVUMAPCSSZ2_2')
NF[['rev_umap_cssz2']]  <- CreateDimReducObject(embeddings = embed, key = "REVUMAPCSSZ2_", assay = DefaultAssay(NF))
NF@reductions[["rev_umap_cssz2"]]@global <- T

plot8 <- DimPlot(NF, group.by = "orig.ident", reduction = "rev_umap_cssz2", cols = ColorBlind[c(10,1,9,3,8,6)], pt.size = 0.9, order = rev(CellTypeOrder)) +
  labs(title = "", x = "UMAP1", y = "UMAP2")
plot8

plot8b <- DimPlot(NF, group.by = "orig.ident", reduction = "rev_umap_cssz2", cols = ColorBlind[c(11,11,11,3,6)], pt.size = 0.9, order = rev(CellTypeOrder)) +
  labs(title = "", x = "UMAP1", y = "UMAP2")
plot8b
...
plot_density(NF, 'Nfil3', pal = 'magma', size = 2)
plot_density(NF, 'Flt3', pal = 'magma', size = 2)
plot_density(NF, 'Tcf7', pal = 'magma', size = 2)
plot_density(NF, 'Zbtb16', pal = 'magma', size = 2)
plot_density(NF, 'Bcl11b', pal = 'magma', size = 2)
plot_density(NF, 'Cd74', pal = 'magma', size = 2)
...
plot8 <- DimPlot(data, group.by = "orig.ident", reduction = "rev_umap_cssz2", cols = ColorBlind[c(10,1,9,3,6,8)], pt.size = 0.9, order = rev(CellTypeOrder)) +
  labs(title = "", x = "UMAP1", y = "UMAP2")
plot8

plot8b <- DimPlot(data, group.by = "orig.ident", reduction = "rev_umap_cssz2", cols = ColorBlind[c(11,11,11,3,6,8)], pt.size = 0.9, order = rev(CellTypeOrder)) +
  labs(title = "", x = "UMAP1", y = "UMAP2")
plot8b

plot9 <- DimPlot(data, reduction = "rev_umap_cssz2", group.by = "seurat_clusters", 
                 label = TRUE, repel = TRUE, 
