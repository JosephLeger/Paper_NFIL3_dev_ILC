...
dir.create(file.path(paste0(PATH, '/Figures/CUT&RUN'), 'Dotplots'))
dir.create(file.path(paste0(PATH, '/Figures/CUT&RUN/Curves'), 'WT'))
dir.create(file.path(paste0(PATH, '/Figures/CUT&RUN/Curves'), 'KO'))
dir.create(file.path(paste0(PATH, '/Saves'), 'CUT&RUN'))

# Set up path for figures and saves
PATH_FIG  <- paste0(PATH, '/Figures/CUT&RUN')
PATH_SAVE <- paste0(PATH, '/Saves/CUT&RUN')
...
HA_cand_enriched_down_peaks <- HA_cand_enriched_peaks[
  HA_cand_enriched_peaks$Gene.Name %in% Shared_DOWN,]
HA_cand_enriched_up_peaks   <- HA_cand_enriched_peaks[
  HA_cand_enriched_peaks$Gene.Name %in% Shared_UP,]
...
write.table(HA_cand_enriched_down_bed, paste0(
  PATH_SAVE, '/HA_cand_enriched_down_peaks_input.bed'),
            sep = '\t', quote = F, row.names = F, col.names = F)
write.table(HA_cand_enriched_up_bed, paste0(
  PATH_SAVE, '/HA_cand_enriched_up_peaks_input.bed'),
            sep = '\t', quote = F, row.names = F, col.names = F)
...
for(i in 1:3){
  box_gfp <- NF_Table$mean_GFP[
    NF_Table$Symbol %in% unique(Transient_regions$Symbol[
      Transient_regions$Cluster %in% paste0('cluster_', i)])]
  box_nf  <- NF_Table$mean_NFIL3[
    NF_Table$Symbol %in% unique(Transient_regions$Symbol[
      Transient_regions$Cluster %in% paste0('cluster_', i)])]
